    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <a href="{{ route('users.create') }}" class="w-full md:w-auto bg-gradient-to-r from-pink-600 to-rose-600 text-white px-6 py-3 rounded-xl hover:from-pink-700 hover:to-rose-700 shadow-lg transform hover:scale-105 transition font-bold flex items-center justify-center gap-2">
                ...
                <table class="min-w-full">
                    <thead>
                        <tr class="border-b border-secondary-200 dark:border-primary-800">
                            ...
                            <th class="px-6 py-4 text-left text-xs font-bold text-secondary-600 dark:text-secondary-400 uppercase tracking-wider">Ciclo</th>
                        ...
                        <tr class="hover:bg-secondary-50 dark:hover:bg-primary-800/50 transition">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-secondary-500 dark:text-secondary-400">
                                    @php
                                        // Obtener último ciclo registrado (asumiendo que queremos el más reciente)
                                        $lastRegistration = $user->registrations->sortByDesc('created_at')->first();
                                        $ciclo = $lastRegistration ? $lastRegistration->course : 'N/A';
                                    @endphp
                                    {{ Str::limit($ciclo, 20) }}
                                </div>
                            </td>
            ...
            <div class="md:hidden space-y-4">
                @forelse ($users as $user)
                    <div class="bg-white dark:bg-primary-900 rounded-xl shadow-md p-5 border border-secondary-200 dark:border-primary-800">
                        <div class="flex items-start justify-between mb-4">
                            <div>
                                <h3 class="font-bold text-secondary-900 dark:text-white text-lg leading-tight">{{ $user->name }}</h3>
                            ...
                            <!-- Ciclo Display -->
                             <div class="text-sm text-secondary-500 dark:text-secondary-400">
                                @php
                                    ...
                                    $ciclo = $lastRegistration ? $lastRegistration->course : 'N/A';
                                @endphp
                                <span class="font-bold mr-1">Ciclo:</span> {{ Str::limit($ciclo, 20) }}
                            </div>

                            @if($user->phone)
                                <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $user->phone) }}" target="_blank" class="flex items-center text-sm text-green-600 dark:text-green-400 font-medium hover:text-green-700 transition-colors">
                                    <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/></svg>
                                    {{ $user->phone }}
                                </a>
                            @else
                                <span class="text-sm text-secondary-500 dark:text-secondary-400 italic">Sin teléfono registrado</span>
                            @endif
                        </div>

                        <div class="pt-3 border-t border-secondary-100 dark:border-primary-800">
                ...
                @empty
                    <div class="text-center p-8 bg-white dark:bg-primary-900 rounded-lg border border-dashed border-secondary-300 dark:border-primary-700">
                        <p class="text-secondary-500 dark:text-secondary-400">No hay usuarios registrados.</p>
                    </div>
                @endforelse
